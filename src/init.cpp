 ...
 */
static const char* BITCOIN_PID_FILENAME = "bfscoind.pid";
...
{
    const std::string URL_SOURCE_CODE = "<https://github.com/bfscoin/bfscoin>";
    const std::string URL_WEBSITE = "<https://bfscoin.org>";
        ...
        do {
            ...
            try {
                if (!is_coinsview_empty) {
                    ...
                    if (fCoinDBUpgraded && ::ChainActive().Height() >= chainparams.GetConsensus().BFSIP002LimitBindPlotterHeight) {
                        LogPrintf("Upgrading UTXO database");

                        CBlockIndex *pindex = WITH_LOCK(cs_main, return ::ChainActive()[chainparams.GetConsensus().BFSIP002LimitBindPlotterHeight]);
                        CValidationState state;
